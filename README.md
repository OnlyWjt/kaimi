...
绕过 `up.sh` 直接 `docker compose` 也能跑。compose 默认绑这台生产机的 docker 网桥 `172.26.0.1:3100`，1Panel 反代填 `http://172.26.0.1:3100`。换机器或网桥段变了，在 `.env` 里设 `KAIMI_BIND_ADDR`，或继续用 `up.sh` 让它现算。别改成 `0.0.0.0`：外部可以绕过代理直连应用、随便伪造 `X-Forwarded-For`，按 IP 的限流就废了。
